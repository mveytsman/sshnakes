defmodule SSHnakes.SSH.Session do
  use GenServer
  ...
  def start_session(user, addr) do
    ...
    {:ok, session_pid} = DynamicSupervisor.start_child(SSHnakes.SSH.Session.Supervisor, {SSHnakes.SSH.Session, [user, addr]})
  ...
  defp initialize_io_loop(user, addr, session_pid) do
    ...
    IO.puts("Welcome to Elixir SSHnakes #{user}! #{Mix.Project.config[:version]}")
    SSHnakes.SSH.Session.input_loop({user, addr, session_pid})
  end
  ...
  def input_loop({user, _addr, pid} = state) do
    case IO.gets("<#{user}> ") do
      ...
      msg ->
        #SSHnakes.SSH.Room.message(pid, String.trim(String.Chars.to_string(msg)))
        SSHnakes.SSH.Session.input_loop(state)
  ...
  def init({:ok, user, _addr}) do
    ...
    #SSHnakes.SSH.Room.register(self(), "#{user}")
    {:ok, []}
  end
