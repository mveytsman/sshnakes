defmodule SSHnakes.TTYClient do
  use GenServer
  ...
  def init(_args) do
    ...
    send(self, :tick)
    {:ok, %TTYClient{port: port}}
  end

  def handle_info({port, {:data, data}}, %TTYClient{port: port} = state) do
  ...
  def handle_info(:tick, %TTYClient{port: port} = state) do
